...
export async function recall(text, k = 4){
  try{
    ...
    const hits = [...byPath.values()].filter(r => r.similarity >= THRESH && !r.path.startsWith(EXCLUDE_RECALL)).sort((a,b)=>b.similarity-a.similarity).slice(0,k);
    for(const r of hits){ try{ const t = fs.readFileSync(absPath(r.path),"utf8").replace(/^---[\s\S]*?---/,"").replace(/\s+/g," ").trim(); r.snippet = t.slice(0,280); }catch(_){ r.snippet = ""; } }
    return hits;
  }catch(_){ return []; }
}
...
export async function ingest(userText, answer){
  ...
  const body = `---\ntipo: inbox\ncreated: ${now.toISOString()}\nconfidence: 0.4\nverificato: false\ntag: ${tags}\n---\n\n# ${titolo}\n\n${fatto}\n\n> Origine: scambio JARVIS del ${now.toLocaleString("it-IT")}. Da promuovere o potare dal verificatore 3:00.\n`;
...
function upsertTopic(topic, coOccur, now){
...
export async function observe(userText, answer){
  try{
    ...
    const now = new Date();
    try{ fs.mkdirSync(LOGS, { recursive:true }); fs.appendFileSync(INTERACT_LOG, JSON.stringify({ ts:now.toISOString(), hour:now.getHours(), dow:now.getDay(), topics, len:(userText||"").length }) + "\n"); }catch(_){}
    try{ fs.mkdirSync(INTERACT_DIR, { recursive:true }); for(const t of topics) upsertTopic(t, topics, now); }catch(_){}
    return { topics };
