...
- **Decisione (2026-06-03):** si procede con lo **spike di OpenClaw usando Claude come backend** (Fra: "proviamo con Claude, visto che è già sviluppato"). Via preferita: **backend Claude CLI** — OpenClaw lancia il `claude.exe` già autenticato in modalità print, **senza API key** (nessun segreto da gestire/rotare). Guida di capacità completa in [[CAP_OPENCLAW]]. Spike: installare (nativo Windows, stesso host del login Claude), puntare al backend claude-cli, far eseguire un task reale, misurare affidabilità/velocità/attriti; poi decidere se promuoverlo a motore agentico di JARVIS o ripiegare su orchestratore su misura (Qwen3 tool-calling). L'opzione Ollama locale resta per la modalità offline.
- **Esito spike (2026-06-03): SUPERATO.** OpenClaw nativo Windows, Gateway `127.0.0.1:18789`, modello `anthropic/claude-sonnet-4-6`, backend Claude. Task reale eseguito da solo (cartella + script Python + run + output), tool-calling affidabile. OpenClaw è un candidato valido a motore agentico di JARVIS. Prossimi passi: (1) sicurezza — `openclaw security audit`, disattivare `controlUi.allowInsecureAuth`, impostare `plugins.allow` e togliere whatsapp; (2) integrazione — agganciare lo switch localMode di JARVIS al model-switch di OpenClaw (Claude online / Ollama offline) + fallback chain; (3) provare il path Ollama locale e un task multi-step. Dettagli in [[CAP_OPENCLAW]].
